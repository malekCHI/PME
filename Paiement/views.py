...
from Paiement.utils import add_paiement, calculate_etat_paiement, TypeStatut, calculate_total_paid_current_month

paiement = Blueprint("paiement", __name__, url_prefix="/paiement")


@paiement.get('/recemment_paye')
def total_paid_current_month_route():
    total = calculate_total_paid_current_month()
    return jsonify({"total_paid_current_month": total}), 200
...
def create_paiement():
    ...
    if montant > facture.total_ttc:
        raise Exception(
            "Le montant du paiement ne doit pas dépasser le montant total de la facture (TTC).")

      # Calculer le montant restant à payer
    montant_restant = facture.total_ttc - \
        sum(paiement.montant for paiement in facture.paiements)

    # Vérifier si le montant est supérieur au montant restant à payer
    if montant > montant_restant:
        raise Exception(
            f"Le montant du paiement ne doit pas dépasser le montant restant à payer de la facture. Montant restant à payer : {montant_restant}.")
    ...
    if statut is not None:
      ...
      # Si la fonction retourne None, vous pouvez lever une exception si nécessaire
        raise Exception(
            f"Impossible de déterminer le paiement de la facture. Montant restant à payer : {montant_restant}.")
    db.session.commit()
