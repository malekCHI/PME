...
def create_facture(id_facture, TypeFacture, date_emission, description, prix_ht, prix_forfaitaire, prix_jour, nbr_jour, prix_livrable, total, tva, total_ttc):
    if TypeFacture == 'FORFAIT':
        ...
        facture = FactureModel(id_facture=id_facture, TypeFacture=TypeFacture, date_emission=date_emission, description=description, prix_ht=prix_ht, prix_livrable=prix_livrable, total=total, tva=tva, total_ttc=total_ttc)
        facture.save_to_db()
        return id_facture
    else:
        return None
